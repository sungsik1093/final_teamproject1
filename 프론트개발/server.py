...
def mypage():
    if not session.get('logged_in'):
        return redirect(url_for('main_page'))  # 로그인하지 않은 경우 메인 페이지로 리다이렉트
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if response.status_code == 201:
                # 성공 시 사용자 데이터를 세션에 저장
                user_info = response.json()  # Node.js에서 반환한 사용자 데이터
...
def login():
    ...
    # Node.js 백엔드로 로그인 요청 보내기
    try:
        response = requests.post(api_url, json={'userNum': userNum, 'password': password})
        response.raise_for_status()  # HTTP 오류 발생 시 예외 처리

        # Node.js 백엔드에서 반환된 응답
        if response.status_code == 200:
            # 로그인 성공 시 클라이언트에 성공 메시지 반환
            return {"success": True, "message": "로그인 성공"}
        else:
            # 로그인 실패 시 실패 메시지 반환
            return {"success": False, "message": response.json().get('message', '로그인 실패')}
    except requests.exceptions.RequestException as e:
        print("API 호출 실패:", e)
...
def register_club():
    ...
    api_url = #'https://teamproject-jv72.onrender.com/api/register_club'
    try:
        response = requests.post(api_url, json=data)  # 데이터를 Node.js로 전송
        response.raise_for_status()

        if response.status_code == 201:  # Node.js API가 성공적으로 처리한 경우
...
def get_club_details():
    # 클라이언트가 보낸 ID를 가져옵니다.
    club_id = int(request.args.get('id'))

    # ID로 동아리 검색
    club = next((c for c in club_list if c['id'] == club_id), None)

    if club:
        return club  # JSON 형식으로 동아리 데이터를 반환
    else:
        return {"error": "동아리를 찾을 수 없습니다."}, 404
...
def change_password():
    ...
    userNum = session['user']['userNum']  # 세션에서 사용자 ID 가져오기
...
def update_info():
    ...
    try:
        ...
        if response.status_code == 200:
            # 성공 시 세션 업데이트
            session['user'].update(updated_info)
